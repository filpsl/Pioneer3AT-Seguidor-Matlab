...
void print_usage(int argc, const char *argv[]) {
  printf("Usage:\n"
         ...
         " The T1 default ipaddr is 192.168.11.2,and the port NO.is 8089. "
...
bool checkSLAMTECLIDARHealth(ILidarDriver *drv) {
  ...
  if (SL_IS_OK(op_result)) { // the macro IS_OK is the preperred way to judge
    ...
    if (healthinfo.status == SL_LIDAR_STATUS_ERROR) {
      fprintf(stderr, "Error, slamtec lidar internal error detected. Please "
...
int main(int argc, const char *argv[]) {
  ...
  // Configuração do UDP para enviar para o MATLAB (Localhost, porta 5000)
  int sockfd = socket(AF_INET, SOCK_DGRAM, 0);
  struct sockaddr_in matlab_addr;
  memset(&matlab_addr, 0, sizeof(matlab_addr));
  matlab_addr.sin_family = AF_INET;
  matlab_addr.sin_port = htons(5000); // Porta que o MATLAB vai escutar
  matlab_addr.sin_addr.s_addr = inet_addr("127.0.0.1"); // Seu próprio PC
  ...
  if (strcmp(opt_is_channel, "--channel") == 0) {
    ...
    if (strcmp(opt_channel, "-s") == 0 ||
      ...
      useArgcBaudrate = true;
               ...
               strcmp(opt_channel, "--tcp") == 0) {
      // read ip addr from the command line...
      opt_channel_param_first =
          argv[3]; // or set to a fixed value: e.g. "192.168.11.2"
      if (argc > 4)
  ...
  if (opt_channel_type == CHANNEL_TYPE_SERIALPORT) {
    ...
    _channel =
        *createUdpChannel(opt_channel_param_first, opt_channel_param_second);
    if (SL_IS_OK((drv)->connect(_channel))) {
      op_result = drv->getDeviceInfo(devinfo);
  ...
  } else if (opt_channel_type == CHANNEL_TYPE_TCP) {
    _channel =
        *createTcpChannel(opt_channel_param_first, opt_channel_param_second);
    if (SL_IS_OK((drv)->connect(_channel))) {
      op_result = drv->getDeviceInfo(devinfo);
  ...
  while (1) {
    ...
    if (SL_IS_OK(op_result)) {
      ...
      for (int pos = 0; pos < (int)count; ++pos) {
        ...
        char buffer[64];
        sprintf(buffer, "%.2f, %.2f, %i", angle, dist, quality);

        sendto(sockfd, buffer, strlen(buffer), 0,
               (struct sockaddr *)&matlab_addr, sizeof(matlab_addr));
      }
    }
...
on_finished:
  if (sockfd != -1)
    close(sockfd); // Fecha o soquete se ele foi aberto
