...
simTime         = 3;         % duração da simulação [s] (2 minutos)
nSteps          = round(simTime * 6);

ref_dist         = 0.55;          % setpoint da distância lateral [cm]

vel_base         = 0.10; % velocidade fixa da roda direita
omega_max        = 0.20;
fact_vel         = 2.0;        % fator de escala da velocidade
...
for i = 1:nSteps
    ...
    vel_x = (v_left + v_right) / 2;
    vel_z = (v_right - v_left) / 2;

    mod = sqrt(vel_x^2 + vel_z^2)
...
teleportModel(worldName, modelName, 0, -1.0, 0.15, 0);
pause(0.5);
